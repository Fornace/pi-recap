...
const DOT_DIM = parseHex("#3a3a52");
const DOT_PEAK = parseHex("#cba6f7");
// 256-color fallback steps from dim -> accent.
const DOT_FALLBACK_TOKENS = [
    "borderMuted",
    "borderMuted",
    "border",
    "accent",
...
export class StatusWidget {
    ...
    firstDeltaAfterRegistration = true;
    tui;
    currentTheme;
    disposed = false;
    ...
    toggleFocus() {
        ...
        if (!this.focused) {
            ...
            this.previousFocus = this.tui.focusedComponent;
    ...
    render(width) {
        ...
        lines.push(this.renderTopBorder(theme, width, goal, state.lastModel, state.modelOverride, liveNotice?.text));
    ...
    hasAnimating() {
        ...
        for (const anim of this.entryAnim.values()) {
            if (anim.finalizedAt === undefined)
                return true; // streaming
            if ((now - anim.finalizedAt) < SETTLE_MS)
                return true; // mid-settle
        }
        const notice = getActiveState().notice;
        if (notice && notice.expiresAt > now)
            return true;
        return false;
    }
    // ── Borders ───────────────────────────────────────────────────────
     ...
     */
    renderTopBorder(theme, width, goal, lastModel, modelOverride, noticeText) {
        ...
        if (noticeText) {
            ...
            if (modelId)
                buildTag(modelOverride ? `${modelId} *` : modelId, "dim");
        }
     ...
     */
    renderBottomBorder(theme, width, totalEntries) {
        const borderColor = (s) => theme.fg("border", s);
    ...
    renderEntryInterior(theme, innerWidth, entry, slot, isSelected) {
    ...
    renderStreamingInterior(theme, innerWidth, entry, anim, now) {
    ...
    renderSettlingInterior(theme, innerWidth, entry, settleStart, isSelected, now) {
    ...
    dotInTimeCell(theme, now, startedAt) {
        const elapsed = now - startedAt;
        ...
        if (theme.getColorMode() === "truecolor") {
            ...
            const stepIdx = Math.min(DOT_FALLBACK_TOKENS.length - 1, Math.floor(k * (DOT_FALLBACK_TOKENS.length - 1) + 0.5));
    ...
    speakerCellStyled(theme, speaker) {
        if (!speaker)
            return "   ";
    ...
    recapForSlot(theme, recap, slot, innerWidth) {
        ...
        if (slot === "newest")
            return colorText(newestColor(), safe);
        if (slot === "mid")
            return theme.fg("muted", safe);
        return theme.fg("dim", safe);
    }
    streamText(theme, raw, innerWidth) {
    ...
    // ── Animation helpers ─────────────────────────────────────────────
    caretVisible(now, lastDeltaAt) {
        if (now - lastDeltaAt < CARET_SOLID_AFTER_DELTA_MS)
            return true;
        const phase = (now % CARET_BLINK_MS) / CARET_BLINK_MS;
        return phase < 0.5;
    }
    slotFor(globalIdx, total) {
    ...
    ensureSlowTimer(hasRecentEntry) {
        if (this.disposed)
            return;
